...
Workflow Storage is configured in `.ftk/config.yaml`. `workflow.backend` selects where newly created Specs are written:
...
When storage-specific mechanics are needed, load the FTK storage reference with:

`node ~/.config/flutter-tools-kit/bin/ftk-run-script.js skills/ftk-config/scripts/show-storage-reference.js --storage <local|github>`
